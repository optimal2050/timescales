...
#
#   calendar_join()        slice-keyed data.frame + timeframe columns (no
#                          ggplot2 — the tidy-workflow attachment helper)
#   geom_calendar()        datetime-keyed data -> slices -> aggregated tiles
#   geom_calendar_tile()   slice-keyed data -> aggregated tiles
...
#'
#' @param x A `data.frame` with a column of slice IDs.
...
#'   `weight` appended. Rows whose key is not a slice of the calendar get
#'   `NA`s (with a warning).
#'
#' @examples
#' cal <- calendar("m12_h24")
#' x <- data.frame(slice = S7::prop(cal, "leaves")$slice, load = 1)
#' head(calendar_join(x, cal))
#' @export
calendar_join <- function(x, calendar, key = "slice", timeframes = NULL,
                          as_factor = TRUE) {
  if (!is.data.frame(x)) {
  ...
  mi <- match(as.character(x[[key]]), leaves$slice)
  if (all(is.na(mi))) {
    .stop("no rows of `x` matched the calendar's slices; check `key=`")
  }
  unknown <- unique(as.character(x[[key]])[is.na(mi)])
  if (length(unknown) > 0L) {
    .warn("%d code(s) in `x$%s` are not slices of the calendar: %s",
...
#' Aggregate a value on slices into an x/y tile frame
#'
#' `by_df` columns (facet/grouping carriers) are preserved: aggregation
#' happens within each distinct combination.
#' @noRd
.heat_aggregate <- function(slices, z, calendar, x_tf, y_tf, fun,
                            by_df = NULL) {
  leaves <- S7::prop(calendar, "leaves")
  tfs    <- S7::prop(calendar, "timeframes")
  levels <- S7::prop(calendar, "levels")
...
#'   (`datetime=`) and a measured column (`z=`); instants are cut to slices
#'   via [`instant_to_slice()`] and aggregated with `fun`.
#' * `geom_calendar_tile()` — **slice mode**: name a slice-ID column
#'   (`slice=`) and the measured column (`z=`).
...
#'   Default `"datetime"`.
#' @param slice Name of the slice-ID column (`geom_calendar_tile()`).
#'   Default `"slice"`.
...
#'   next-finest on x.
#' @param fun Aggregator over instants/slices falling in one tile. Default
#'   `mean`.
...
#'
#'   # slice mode
#'   y <- data.frame(slice = S7::prop(cal, "leaves")$slice, v = 1:288)
#'   ggplot(y) +
...
geom_calendar <- function(calendar, z,
  ...
  build <- function(d) {
    ...
    slices <- instant_to_slice(dtm, calendar)
    .heat_aggregate(slices, d[[z]], calendar, x_tf, y_tf, fun,
...
geom_calendar_tile <- function(calendar, z,
  ...
  .check_calendar(calendar)
  build <- function(d) {
    .check_layer_cols(d, c(slice, z, by))
    .heat_aggregate(as.character(d[[slice]]), d[[z]], calendar,
                    x_tf, y_tf, fun,
                    by_df = if (length(by)) d[by] else NULL)
  }
  .calendar_tile_layer(build, data, z, ...)
