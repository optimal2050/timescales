...
test_that("calendar_catalog() lists 37 designs with consistent metadata", {
  ...
  expect_named(cat_df, c("id", "tokens", "timeframes", "n_slices",
...
test_that("every catalog id builds with the advertised slice count and
           shares summing to 1", {
  cat_df <- calendar_catalog()
  for (i in seq_len(nrow(cat_df))) {
    cal <- calendar(cat_df$id[i])
    leaves <- S7::prop(cal, "leaves")
    expect_equal(nrow(leaves), cat_df$n_slices[i], info = cat_df$id[i])
    expect_equal(sum(leaves$share), 1, tolerance = 1e-9,
                 info = cat_df$id[i])
  }
})

test_that("well-known slice counts hold", {
  ...
  cat_df <- calendar_catalog()
  got <- stats::setNames(cat_df$n_slices, cat_df$id)[names(expected)]
...
test_that("m12_md365 has ragged months and drops Feb 29", {
  ...
  expect_equal(instant_to_slice(lubridate::ymd("2020-02-29"), md),
               NA_character_)
  expect_equal(instant_to_slice(lubridate::ymd("2021-03-15"), md),
               "m03_d15")
})

test_that("m12_md366 covers Feb 29; m12_md360 drops day 31", {
  md366 <- calendar("m12_md366")
  expect_equal(instant_to_slice(lubridate::ymd("2020-02-29"), md366),
               "m02_d29")
  md360 <- calendar("m12_md360")
  expect_equal(instant_to_slice(lubridate::ymd("2021-01-31"), md360),
               NA_character_)
  expect_equal(instant_to_slice(lubridate::ymd("2021-01-30"), md360),
               "m01_d30")
})
...
test_that("m12_md365_h24 nests hours inside ragged days", {
  ...
  expect_equal(instant_to_slice(dtm, cal), "m02_d28_h13")
})
...
test_that("type-axis calendars map instants to composite slices", {
  dtm <- lubridate::ymd_h(c("2021-01-15 03", "2021-01-16 18"), tz = "UTC")
  expect_equal(instant_to_slice(dtm, calendar("s4_hp3")),
               c("WIN_NIGHT", "WIN_PEAK"))
  expect_equal(instant_to_slice(dtm, calendar("wk2_h24")),
