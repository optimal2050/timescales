...
```{r scales-trio, fig.width = 7, fig.height = 2.8, eval = viz}
...
```{r stack-structure, fig.width = 7, fig.height = 4, eval = viz}
...
(`rule =` and `year =` are the `recast_calendar()` arguments; the base
grid is hourly by default), so the whole stack shares one continuous
scale — the annual mean on top, the finest resolution at the bottom.
`labels =` names timeframes whose member names are written on the
plane, and `palette = NULL` lets you attach your own fill scale:

```{r stack-data, fig.width = 7, fig.height = 4, eval = viz}
