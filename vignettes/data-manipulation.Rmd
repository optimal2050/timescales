...
| `register_rule()` / `register_calendar_map()` / `register_conversion()` | registries | per-column rules, exact crosswalks, functional overrides |
...
grid derivation for that pair; `register_conversion()` goes one step
